<?php
/**
 * Plugin Name: Carousel CK
 * Plugin URI: http://www.ceikay.com/plugins-wordpress/carousel-ck
 * Description: Carousel CK is a responsive slideshow plugin that show your images with a nice carousel effect.
 * Version: 1.1.0
 * Author: Cédric KEIFLIN
 * Author URI: http://www.ceikay.com/
 * License: GPL2
 */
...
class Carouselck {
	...
	function __construct() {
		$this->pluginname = 'carousel-ck';
		$this->pluginurl = plugins_url('', __FILE__);
		$this->plugindir = WP_PLUGIN_DIR . '/' . $this->pluginname;
		$this->settings_field = 'carousel-ck_options';
		$this->options = get_option($this->settings_field);
		$this->prourl = 'http://www.ceikay.com/en/wordpress-plugins/carousel-ck';
		// add the needed classes
		if (!class_exists("CKfolder"))
			require($this->plugindir . '/cklibrary/class-ckfolder.php');

		if (!class_exists("Carouselck_CKfields"))
			require($this->plugindir . '/cklibrary/class-ckfields.php');
		$this->ckfields = new Carouselck_CKfields();
	}

	function init() {
		if ( file_exists(WP_PLUGIN_DIR . '/carousel-ck-pro-addon/carousel-ck-pro-addon.php') && is_plugin_active('carousel-ck-pro-addon/carousel-ck-pro-addon.php') ) {
			require(WP_PLUGIN_DIR . '/carousel-ck-pro-addon/carousel-ck-pro-addon.php');
			$this->ispro = true;
			$this->pro_class = new CarouselckProAddon();
		}
		...
		// create the widget
		add_action('widgets_init', array($this, 'create_carouselck_widget'));
	}
	...
	function set_admin_menu_image_position() {
		?>
		<style type="text/css">#toplevel_page_carouselck_general .wp-menu-image > img { padding: 12px 0 0 !important; }</style>
		<?php
	}
	 ...
	 */
	public function create_carouselck_widget() {
		require_once( $this->plugindir . '/includes/widget-carouselck.php' );
		register_widget('carouselck_widget');
	}

	/**
	 * Create the carouselck post type
	 */
	function create_post_type() {
		register_post_type('carouselck', array(
			'labels' => array(
				'name' => __('Carousel CK'),
				'singular_name' => __('Carousel CK'),
	...
	function create_admin_menu() {
		$this->pagehook = $page = add_menu_page('Carousel CK', 'Carousel CK', 'administrator', 'carouselck_general', array($this, 'render_general'), $this->pluginurl . '/images/admin_menu.png');
		add_submenu_page('carouselck_general', __('Carousel CK'), __('All Carousels'), 'administrator', 'carouselck_general', array($this, 'render_general'));
		$editpage = add_submenu_page('carouselck_general', __('Edit'), __('Add New'), 'administrator', 'carouselck_edit', array($this, 'render_edit'));
	...
	function load_admin_assets() {
		...
		<script type="text/javascript">
			function ckdoajax(func, id) {
				...
				jQuery('#carouselck_admin').html('<div class="ckwait_overlay"></div>');
				// since 2.8 ajaxurl is always defined in the admin header and points to admin-ajax.php
				jQuery.post(ajaxurl, data, function(response) {
					jQuery('.ckwait_overlay').remove();
					jQuery('#carouselck_admin').html(response);
				});
			}
		</script>
		<?php
	...
	function render_general() {
		?>
		<div id="carouselck_admin">
			<?php
			require_once($this->plugindir . '/includes/carouselck_general.php');
			?>
		</div>
		<?php
	}
	...
	function render_edit() {
		require_once($this->plugindir . '/includes/carouselck_edit.php');
	}
	...
	function render_slideshow($id) {
		$params = json_decode(str_replace('|qq|', '"', get_post_meta($id, 'carousel-ck-params', TRUE)));
		$this->params = $params;
		...
		ob_start();
		?>
		<div class="carouselck carouselck_wrap <?php echo $this->get_param('skin'); ?>" id="carouselck_wrap_<?php echo $id; ?>"<?php echo $width; ?>>
			<?php
			for ($i = 0; $i < count($items); ++$i) {
				...
				$dataalbum = ($this->get_param('lightboxgroupalbum', '0')) ? '[albumcarouselck' . $module->id . ']' : '';
				$datarel = ($imgtarget == 'lightbox') ? 'data-rel="lightbox' . $dataalbum . '" ' : '';
				$datatime = ($item->imgtime) ? ' data-time="' . $item->imgtime . '"' : '';
				?>
				<div class="carouselck" <?php echo $datarel . $datatitle; ?>data-thumb="<?php echo get_site_url() . '/' . trim($item->imgthumb, '/'); ?>" data-src="<?php echo get_site_url() . '/' . trim($item->imgname, '/'); ?>" <?php
				if ($item->imglink)
				...
				?>>
					...
					if (($item->title || $item->description || $item->article) && (($this->get_param('lightboxcaption', 'caption') != 'title' || $imgtarget != 'lightbox') || !$item->imglink)) {
						?>
						<div class="carouselck_caption <?php echo $this->get_param('captioneffect', 'moveFromBottom') ?>">
	...
	function load_slideshow_assets($id) {
		...
		$image_css = $this->create_css('image_');
		/*
		...
		wp_enqueue_script('carouselck', $this->pluginurl . '/assets/carouselck.js');
		?>
		<script type="text/javascript"> <!--
			jQuery(function() {
				jQuery('#carouselck_wrap_<?php echo $id; ?>').carouselck({
					wrapheight: '<?php echo $this->get_param('wrapheight', '40') ?>',
					imageheight: '<?php echo $this->get_param('imageheight', '62') ?>',
					imagesratio: '<?php echo $this->get_param('imagesratio', '0.72') ?>',
					pagination: <?php echo $this->get_param('pagination', '1') ?>,
					thumbnails: <?php echo $this->get_param('thumbnails', '1') ?>,
					playPause: '<?php echo $this->get_param('playPause', '1') ?>',
					duration: <?php echo $this->get_param('duration', '600') ?>,
					time: <?php echo $this->get_param('time', '7000') ?>,
					captionduration: <?php echo $this->get_param('captionduration', '1500') ?>,
					autoAdvance: <?php echo $this->get_param('autoAdvance', '1') ?>,
					lightbox: '<?php echo $this->get_param('lightboxtype', 'mediaboxck') ?>',
					<?php echo $navigation ?>
					hover: <?php echo $this->get_param('hover', 'true') ?>
					});
				}); //--> </script>

		<link href="<?php echo $this->pluginurl ?>/themes/<?php echo $theme ?>/css/carouselck.css" rel="stylesheet" type="text/css" />
		<style type="text/css">
			#carouselck_wrap_<?php echo $id; ?> .carouselck_pag_ul img {
				height:<?php echo $this->test_unit($this->get_param('thumbnailheight', '75')); ?>;
				width: auto;
			}
			#carouselck_wrap_<?php echo $id; ?> .carouselck_caption {
				display: block;
				position: absolute;
			}
			#carouselck_wrap_<?php echo $id; ?> .carouselck_caption > div {
			...
			#carouselck_wrap_<?php echo $id; ?> .carouselck_caption div.carouselck_caption_title {
				<?php echo $title_css['fontcolor'] . $title_css['fontsize'] . $title_css['fontweight'] . $title_css['fontfamily'] ?>
			}
			#carouselck_wrap_<?php echo $id; ?> .carouselck_caption div.carouselck_caption_desc {
				<?php echo $desc_css['fontcolor'] . $desc_css['fontsize'] . $desc_css['fontweight'] . $desc_css['fontfamily'] ?>
			}
			#carouselck_wrap_<?php echo $id; ?> .carouselck_images > img {
				<?php echo $image_css['borderradius'] . $image_css['shadow'] . $image_css['border'] ?>
			}
		</style>
		<?php
	}
	 ...
	 * 
	 * @param integer $id the carouselck post ID
	...
	protected function get_items($id) {
		...
		switch ( $this->get_param('slides_sources') ) {
			case 'slidesmanager':
			default:
				$items = json_decode(str_replace('|qq|', '"', get_post_meta($id, 'carousel-ck-slides', TRUE)));
				break;
			case 'autoloadfolder':
...
function Carouselck_loadjquery() {
	wp_enqueue_script('jquery');
}

// load the process
$carouselckClass = new Carouselck();
$carouselckClass->init();
...
if (!is_admin()) {
	 ...
	 */
	function do_carouselck($id) {
		echo get_carouselck($id);
	}
	...
	* @param integer $id the slideshow ID
	*/
	function get_carouselck($id) {
		add_thickbox();
		$carouselckClass = new Carouselck();
		return $carouselckClass->render_slideshow($id);
	}

	// load jquery in frontend
	add_action('init', 'Carouselck_loadjquery');
} else {
	/**
...
add_shortcode( 'carouselck', 'shortcode_carouselck' );
 ...
 */
function shortcode_carouselck($attr) {
	if ( isset($attr['id']) ) {
		return get_carouselck( (int) $attr['id'] );
	}
	return null;
}