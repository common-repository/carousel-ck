...
class carouselck_widget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'carouselck_widget', 'Carousel CK', $widget_ops );
	}
	...
	function form($instance) {
		...
		$instance['carouselck_id'] = (int)$posts_carouselck_id[0]->ID;
		?>
		<p>
			<label for="<?php echo $this->get_field_name('carouselck_id') ?>"><?php _e('Select the slideshow to load') ?> :</label>
			<br />
			<select name="<?php echo $this->get_field_name('carouselck_id') ?>" value="0">
	...
	public function widget($args, $instance) {
		if (! $instance) {
			echo '<p style="color:red">Please select a carousel to show in the widget options</p>';
			return;
		}
